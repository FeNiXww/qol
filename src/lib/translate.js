...
export async function translateText(text, fromLang, toLang, knownWords = []) {
  if (!text?.trim()) return text;
  if (fromLang === toLang) return text;
  // Emoji-only messages (e.g. "😂😂", "👍!") need no translation
  if (isEmojiOnly(text)) return text;

  const preserveInstruction = knownWords.length
    ? `\n- The reader already knows these specific words/phrases in ${LANG_NAMES[fromLang] || fromLang} — do NOT translate them, leave them exactly as written in the original: ${knownWords.map(w => `"${w}"`).join(', ')}. Translate the rest of the message normally around them.`
    : '';
...
- If the message is gibberish, random keyboard characters, or has no recognizable meaning in any language, set "translatable" to false and leave "translation" empty. Do NOT transliterate gibberish.${preserveInstruction}`,
